...
#### explicit-vendor-chunk  暴露三方数据块为DLL
...
5. 源代码 [webpack-demo-8](https://github.com/Matthrews/webpack-demos)
...
1. AggressiveSplittingPlugin，用于将包分成多个较小的块以改善缓存。这最适合HTTP2 Web服务器，否则，请求数量增加会产生开销。

2. AggressiveSplittingPlugin 拆分每个块，直到达到指定的maxSize。在此示例中，它尝试使用<50kB的原始代码创建块，通常将其最小化为〜10kB。
它按文件夹结构对模块进行分组，因为同一文件夹中的模块可能具有相似的重复文本，从而使它们高效地压缩在一起。它们也可能一起改变。

3. AggressiveSplittingPlugin 将其拆分记录在webpack记录中。下次运行时，它将尝试使用最后记录的拆分。
...
- 较小的maxSize会改善缓存，因为块更改的频率降低，并且在更新后可以更频繁地重用。

- 最大的maxSize可以改善压缩效果，因为gzip对于较大的文件效果更好。更有可能找到重复的字符串，等等。

- 随着请求数量的减少，向后兼容性（非HTTP2客户端）会随着maxSize的增大而提高。
